<script setup>
	...
	/*
	  tab标签模块 0：待付款；1：待发货；2：已发货/待收货；3：已收货/待评价；4：已完成；-1：已取消；-2：已退款
	*/
	const tabs = ref([{
			status: "all",
			title: t("order.all")
...
<template>
	<header class="sticky top-0 z-10 p-6 px-3 w-dvw back_4 bg-white">
	...
	<div v-if="tabname==='1'" class="grid grid-cols-2 gap-4 justify-items-center mx-3 mt-2 bg-[#F2FAFA] ">
		<div class="flex flex-col my-2 items-center">
			<span class="text-gray-600">{{ $t("order.totalPayment") }}</span><span
				class="text-lg font-semibold text-[#FE4857] mt-3">${{ totalprice }}</span>
		</div>
		<div class="flex flex-col my-2 items-center">
			<span class="text-gray-600">{{ $t("order.earnings") }}</span>
			<span class="text-lg font-semibold text-[#FE4857] mt-3">${{ totalprofit }}</span>
		</div>
	</div>
	<main class="pb-20">
		<div v-if="List.length<=0">
			<van-empty :description="$t('order.empty')" :image="img" image-size="17rem">
			</van-empty>
		</div>
		<van-list v-model:loading="loading" :finished="finished" :immediate-check="false"
			:finished-text="$t('order.noMore')" @load="onload()">
			<div v-for="item in List" class="bg-white mx-3 mt-3 rounded-md back_3 back_4">
				<div class="flex items-center pt-1.5 zt">
					<!-- <van-icon class="mx-3" name="ellipsis" size="24" /> -->
					<span v-if="item.status===0" class="ml-auto  ">{{ $t("order.pendingPayment") }}</span>
					<span v-if="item.is_pick===1 &&item.status===1" class="ml-auto  mr-3">{{
              $t("order.pendingShipment")
            }}</span>
					<span v-if="item.is_pick===0 && item.status===1" class="ml-auto  mr-3">{{
              $t("order.awaitingPickup")
            }}</span>
					<span v-if="item.status===2" class="ml-auto  ">{{ item.deliverystatus }}</span>
					<span v-if="item.status===3" class="ml-auto  ">{{ $t("order.received") }}</span>
					<span v-if="item.status===4" class="ml-auto  ">{{ $t("order.completed") }}</span>
					<span v-if="item.status===-1" class="ml-auto  ">{{ $t("order.canceled") }}</span>
					<span v-if="item.status===-2" class="ml-auto  ">{{ $t("order.refunded") }}</span>
					<div v-if="item.is_pick===0 && item.status===1" class="flex justify-end  ">
						<van-checkbox v-model="item.clicked"
							@click="changeclick(item.order_id,item.total_cost)"></van-checkbox>
					</div>
				</div>
				<div class="flex mx-3" @click="router.push({ path: '/orderDetail', query: { id: item.order_id } })">
					<div class="mt-2">
						<van-image
							...
							height="100" lazy-load radius="10px" width="100">
					...
					<div class="ml-3 mt-3 flex-1 mr-1 flex  flex-col">
						<div class="-mt-2">
							<span class="" style="line-height: 16px;">
						...
						<div class="flex mt-2 mr-2 justify-between items-center ">
							<div class="text-right flex justify-items-start items-center  ">
								<span class="text-[#939393] text-sm  mr-2 ">{{ $t("order.quantity") }}</span>
								<span class="font-semibold  text-sm">{{ item.total_num }}</span>
							</div>
							<div class="text-right flex justify-items-start items-center  ">
								<span class="text-[#939393] text-sm  mr-2">{{ $t("order.totalCost") }}</span>
								<span class="font-semibold  text-sm"> $ {{ item.total_cost }}</span>
							</div>
						</div>
						<div class="flex mt-2 mr-2 justify-between items-center ">
							<div class="text-right flex justify-items-start items-center">
								<span class="text-[#939393] text-sm  mr-2 ">{{ $t("order.sellingPrice") }}</span>
								<span class=" font-semibold  text-sm "> $ {{ item.total_price }}</span>
							</div>
							<div class="text-right flex justify-items-start items-center">
								<span class="text-[#939393] text-sm  mr-2">{{ $t("order.earnings") }}</span>
								<span class="font-semibold  text-sm  text-[#FE4857]"> $ {{ item.total_profit }}</span>
							</div>
						</div>
					</div>
				</div>
              ...
              $t("order.details")
					...
					<span v-if="item.is_pick===0 && item.status===1"
		...
		<div v-if="listQuery.status==='all' || listQuery.status==='5'"
			class="fixed bottom-28 left-12 button_1 w-[100vw] flex justify-center">
			<van-button color="#009996" round size="small" style="width:90%;height: 3.5rem;" @click="add">
				{{ $t("order.pickupAll") }}
			</van-button>
		</div>
	</main>
	...
	* {
	...
	.zt {
		...
		span {
			...
			box-shadow: 0px 5px 30px 0px rgba(20, 26, 82, 0.08);
