...
<template>
  ...
  <header>
    <van-nav-bar :title="$t('withdraw.withdrawal')" :border="false" :left-text="$t('goback')" left-arrow
      ...
      <template #right>
				<span @click="router.push('/withdrawRecord')" class="text-white">{{
            $t("withdraw.withdrawalRecords")
          }}</span>
      </template>
    </van-nav-bar>
  </header>
    ...
    <van-form @submit="onSubmit" class="">
      ...
      <div class="mt-6 bg-white pb-6 ts ">
        ...
        <van-field :placeholder="$t('withdraw.enterAmount')" v-model="price1" @input="changePrice"
          ...
          <template #button>
						<span v-if="extracttype !== $t('withdraw.blockchain')" class="text-blue-500"
                  @click="price1 = ((info.balance * rechargeRate).toFixed(2))">{{
                $t("withdraw.all")
              }}</span>

            <span v-if="extracttype === $t('withdraw.blockchain')" class="text-blue-500"
                  @click="price1 = ((info.balance*1).toFixed(2))">{{
                $t("withdraw.all")
              }}</span>
        ...
        <div class="flex text-blue-500 justify-between mt-3 mx-4 text-sm">
					<span>
						{{ $t("withdraw.actualAmountReceived") }}{{
              price1 == null ? 0 :
