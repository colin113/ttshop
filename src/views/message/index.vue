...
<template>
	<!-- <div class="w-[100vw] h-[100vh] fixed" style="z-index: -2;"></div> -->
	<header class="sticky top-0  bg-white">
		<van-nav-bar :title="$t('message.message2')" :left-text="$t('goback')" left-arrow @click-left="onClickLeft">
		</van-nav-bar>
		<div class="rounded-md w-full">
			<van-tabs v-model:active="active" @click-tab="onClickTab" color="#009996" type="card">
	...
	<main>
		...
		<div>
			<van-list v-model:loading="loading" :finished="finished" :finished-text="$t('message.nomore')"
				...
				<div class="bg-white mx-3 mt-3 rounded-md" v-for="item in List " :key="item.id"
					...
					<div class="flex justify-between mx-3 py-1.5">
						...
						<div class="flex items-center">
							<span
								class="text-green-500">{{ item.type === 1 ? $t("message.message1") : $t("message.notice") }}</span>
						</div>
					</div>
					<div class="flex justify-between mx-3 py-1.5">
