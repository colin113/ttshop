...
const sortText = ref(t("goods.time1"))
const listQuery = ref({
  switch: 'all',
  page: '1',
  limit: '10',
...
const onload = (id) => {
  ...
  show.close();
  listQuery.value.sort = sortType.value
  if (id !== undefined) {
    show.open();
    List.value = [];
...
<template>
  <header class="sticky top-0 z-10 pb-1.5" style="background-color: #fff">
    <van-nav-bar :title="$t('goods.ProductManagement')" :left-text="$t('goback')" left-arrow :border="false"
      ...
      <template #right>
        <van-icon color="#009996" @click="openshow" size="22px" name="todo-list"/>
      </template>
    </van-nav-bar>
    ...
    <div>
      <div class="bar" v-if="isShowBar">
        <div style="margin-left: 10px;font-size: 16px">
          {{ $t('goods.totalNum') }}&nbsp:&nbsp{{ totalNum }}
        </div>
        <div style="display: flex">
          <span style="font-size:14px">{{ $t('goods.order') }}&nbsp:&nbsp{{ sortText }}</span>
        </div>
      </div>
  ...
  <van-action-sheet v-model:show="showFoot" :actions="actions" @select="onSelect"/>
</template>
