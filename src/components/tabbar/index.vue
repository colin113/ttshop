<script setup>
	...
	const list = [{
		...
		// {
		//   text: t("tabbar.financialManagement"),
		//   icon_1: new URL('@/assets/image/tabbar/未选中/余额宝.svg', import.meta.url).href,
		//   icon_2: new URL('@/assets/image/tabbar/选中/余额宝.svg', import.meta.url).href,
		//   path: '/yuebao'
		// },
		{
			text: t("tabbar.order"),
			...
			<van-tabbar-item replace :to="item.path" v-for="(item, index) in list" :key="index">
				<template #icon="{ active }">
					<div class="!flex !flex-col !justify-center">
						...
						<span class="text-center mt-2" :class="{active:active}">{{ item.text }}</span>
					</div>
				</template>
			</van-tabbar-item>
		</van-tabbar>
